    ...
    protected Connection openConnection() throws SQLException, ClassNotFoundException
    {
        ...
        if (dbLocation==null)
        {
            ...
            if (AiMineCore.isTest())
                connection = DriverManager.getConnection("jdbc:mysql://" + this.hostname + ":" + this.port + "/" + this.database+"?zeroDateTimeBehavior=convertToNull&autoReconnect=true&characterEncoding=UTF-8&characterSetResults=UTF-8", this.user, null);
            else
                connection = DriverManager.getConnection("jdbc:mysql://" + this.hostname + ":" + this.port + "/" + this.database+"?zeroDateTimeBehavior=convertToNull&autoReconnect=true&characterEncoding=UTF-8&characterSetResults=UTF-8" , this.user, this.password);
            return connection;
        }
        else
        {
            ...
            connection = DriverManager.getConnection("jdbc:sqlite:" +dataFolder.toPath().toString() + "/" + dbLocation+".db?zeroDateTimeBehavior=convertToNull&autoReconnect=true&characterEncoding=UTF-8&characterSetResults=UTF-8");
            return connection;
        }
    }
